...
diskaddr(u_int blockno)
{
        if (super) {
                if (blockno >= super->s_nblocks) {
                        user_panic("diskaddr blockno too great");
                }
        }
...
map_block(u_int blockno)
{
    ...
    // Step 2: Alloc a page of memory for this block via syscall.
        u_int va = diskaddr(blockno);
        return syscall_mem_alloc(0,va,PTE_R|PTE_V);
}
...
read_block(u_int blockno, void **blk, u_int *isnew)
{
        ...
        if (super && blockno >= super->s_nblocks) {
                user_panic("reading non-existent block %08xn", blockno);
        }
        ...
        if (bitmap && block_is_free(blockno)) {
                user_panic("reading free block %08xn", blockno);
        }
...
free_block(u_int blockno)
{
        ...
        if (blockno == 0) {
                user_panic("the blockno is unvalidn");
        } else {
...
check_write_block(void)
{
        ...
        read_block(1, 0, 0);
        user_assert(strcmp((char *)diskaddr(1), "OOPS!n") == 0);
