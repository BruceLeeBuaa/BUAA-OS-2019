...
{
        writef("serve_open %08x %x 0x%xn", envid, (int)rq->req_path, rq->req_omode);
...
serve(void)
{
        ...
        for (;;) {
                ...
                if (!(perm & PTE_V)) {
                        writef("Invalid request from %08x: no argument pagen", whom);
                ...
                switch (req) {
                        ...
                        default:
                                writef("Invalid request code %d from %08xn", whom, req);
                                break;
                }
...
umain(void)
{
        ...
        writef("FS is runningn");

        writef("FS can do I/On");
