...
#define LIST_INSERT_AFTER(listelm, elm, field) do {                        if((LIST_NEXT((elm),field) = LIST_NEXT((listelm),field)) != NULL){                           LIST_NEXT((elm),field)->field.le_prev = &LIST_NEXT((elm),field);           }                                              LIST_NEXT((listelm),field) = (elm);         (elm)->field.le_prev = &LIST_NEXT((listelm),field);} while (0)
 ...
 */
//elm是指向page结构体的指针；pplink是一个包含着两个指针的结构体
 ...
 */
#define LIST_INSERT_TAIL(head, elm, field) do {                    if(LIST_FIRST((head)) == NULL) {                                     LIST_FIRST((head)) = (elm);                                      LIST_NEXT((elm),field) = NULL;                                  (elm)->field.le_prev = &LIST_FIRST((head));             } else {                                                            typeof(LIST_FIRST((head))) p;                                  (p) = LIST_FIRST((head));                                      while(LIST_NEXT((p),field) != NULL){                                   (p) = LIST_NEXT((p),field);                                }                                                             LIST_INSERT_AFTER((p),(elm),field);                    }                                                 } while (0)
