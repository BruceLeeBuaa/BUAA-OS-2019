...
umain(void)
{
        ...
        if (r == 0) {
                ...
                for (i=0; i<max; i++) {
                        if(pipeisclosed(p[0])){
                                writef("RACE: pipe appears closedn");
                                exit();
                        }
                        syscall_yield();
                }
        ...
        pid = r;
        writef("pid is %dn", pid);
        va = 0;
        kid = &envs[ENVX(pid)];
        writef("kid is %dn", kid-envs);
        dup(p[0], 10);
        while (kid->env_status == ENV_RUNNABLE)
                dup(p[0], 10);

        writef("child done with loopn");
        if (pipeisclosed(p[0]))
        ...
        if (pageref((void*)va) != 3+1)
                writef("nchild detected racen");
        else
                writef("nrace didn't happenn", max);
}
