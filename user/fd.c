...
dev_lookup(int dev_id, struct Dev **dev)
{
        ...
        writef("[%08x] unknown device type %dn", env->env_id, dev_id);
        return -E_INVAL;
}
...
read(int fdnum, void *buf, u_int n)
{
        ...
        if ((fd->fd_omode & O_ACCMODE) == O_WRONLY) {
                writef("[%08x] read %d -- bad moden", env->env_id, fdnum);
                return -E_INVAL;
        }
...
write(int fdnum, const void *buf, u_int n)
{
        ...
        if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) {
                writef("[%08x] write %d -- bad moden", env->env_id, fdnum);
                return -E_INVAL;
        }

        if (debug) writef("write %d %p %d via dev %sn",
