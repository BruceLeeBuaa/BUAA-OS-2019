...
void mips_detect_memory()
{
     ...
     * (When use real computer, CMOS tells us how many kilobytes there are). */
        //0x80000000-0x84000000共64M空间,每个页4k
        maxpa = 0x04000000;
    ...
    printf("base = %dK, extended = %dKn", (int)(basemem / 1024),
           (int)(extmem / 1024));
}
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    /* Step 1: Check if `size` is a multiple of BY2PG. */
        if(size % BY2PG != 0){
                printf("Size is not a multiple of BY2PGn");
        }
    ...
    /* Hint: Use `boot_pgdir_walk` to get the page table entry of virtual address `va`. */
        for(i = 0;i < size/BY2PG;i++){
                ...
                *pgtable_entry = (pa+i*BY2PG)|PTE_V|perm;
        }
...
void mips_vm_init()
{
    ...
    printf("to memory %x for struct page directory.n", freemem);
    mCONTEXT = (int)pgdir;
    ...
    printf("to memory %x for struct Pages.n", freemem);
    ...
    printf("pmap.c:t mips vm init successn");
}
    ...
    /* Step 1: Get a page from free memory. If fails, return the error code.*/
        if((ppage_temp = LIST_FIRST(&page_free_list)) != NULL)
        {
                *pp = ppage_temp;
                ...
                u_long va_pp = page2kva(ppage_temp);
                bzero((void*)va_pp,BY2PG);
                return 0;
        }
        return -E_NO_MEM;
...
page_free(struct Page *pp)
{
     ...
     * so PANIC !!! */
    panic("cgh:pp->pp_ref is less than zeron");
}
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
     ...
     * When creating new page table, maybe out of memory. */
        ...
        if((*pgdir_entryp & PTE_V) == 0x0){
                if(create){
                        int available = page_alloc(&ppage);
                        if(available == 0){
                                u_long ppage_va = page2kva(ppage);
                                pgtable = ppage_va;
                                ppage->pp_ref += 1;
                                *pgdir_entryp = PADDR(pgtable)|PTE_V;
                        } else {
...
page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0) {
        if (pa2page(*pgtable_entry) != pp) {
            page_remove(pgdir, va);//把页表项清零，同时删掉对那个物理页面的引用
        } else  {
...
physical_memory_manage_check(void)
{
    ...
    *temp = 1000;
    // free pp0
    page_free(pp0);
    printf("The number in address temp is %dn",*temp);
    ...
    struct Page *test_pages;
        ...
        for(i=0;i<10;i++) {
                ...
                //printf("0x%x  0x%xn",&test_pages[i], test_pages[i].pp_link.le_next);
        ...
        while(p!=NULL)
        {
                //printf("%d %dn",p->pp_ref,answer1[j]);
                assert(p->pp_ref==answer1[j++]);
                //printf("ptr: 0x%x v: %dn",(p->pp_link).le_next,((p->pp_link).le_next)->pp_ref);
        ...
        //printf("---%dn",test_pages[4].pp_ref);
        LIST_INSERT_AFTER(&test_pages[4], q, pp_link);
        //printf("---%dn",LIST_NEXT(&test_pages[4],pp_link)->pp_ref);

        p = LIST_FIRST(&test_free);
        j=0;
        while(p!=NULL){
        //      printf("%d %dn",p->pp_ref,answer2[j]);
                        assert(p->pp_ref==answer2[j++]);
                        p=LIST_NEXT(p,pp_link);
        }


    printf("physical_memory_manage_check() succeededn");
}
...
page_check(void)
{
    ...
    printf("va2pa(boot_pgdir, 0x0) is %xn",va2pa(boot_pgdir, 0x0));
    printf("page2pa(pp1) is %xn",page2pa(pp1));
    ...
    assert(pp1->pp_ref == 2);
    printf("pp2->pp_ref %dn",pp2->pp_ref);
    assert(pp2->pp_ref == 0);
    printf("end page_insertn");
